...
constexpr float ballRadius{10.0}, ballVelocity{0.3f};
constexpr float paddleWidth{60.0}, paddleHeight{20.0}, paddleVelocity{0.6f};
constexpr float blockWidth{60.0}, blockHeight{20.0};
constexpr int countBlocksX{11}, countBlocksY{4};
constexpr float ftStep{1.0}, ftSlice{1.0};
...
struct Rectangle
{
    ...
    float x() const noexcept { return shape.getPosition().x; }
    float y() const noexcept { return shape.getPosition().y; }
    float left() const noexcept { return x() - shape.getSize().x / 2.0; }
    float right() const noexcept { return x() + shape.getSize().x / 2.0; }
    float top() const noexcept { return y() - shape.getSize().y / 2.0; }
    float bottom() const noexcept { return y() + shape.getSize().y / 2.0; }
};
...
struct Paddle : public Rectangle
{
    ...
    Paddle(float mX, float mY)
    {
        ...
        shape.setOrigin(paddleWidth / 2.0, paddleHeight / 2.0);
    }
...
struct Brick : public Rectangle
{
    ...
    Brick(float mX, float mY)
    {
        ...
        shape.setOrigin(blockWidth / 2.0, blockHeight / 2.0);
    }
};
...
struct Game
{
    ...
    FrameTime lastFt{0.0}, currentSlice{0.0};
    bool running{false};

    // These members are game entities.
    Ball ball{windowWidth / 2, windowHeight / 2};
    Paddle paddle{windowWidth / 2, windowHeight - 150};
    vector<Brick> bricks;
    ...
    void run()
    {
        ...
        while(running)
        {
            ...
            auto ftSeconds(ft / 1000.0);
            auto fps(1.0 / ftSeconds);
